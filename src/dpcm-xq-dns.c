...
static int win_average_buffer (float *samples, int sample_count, int half_width);
...
// reduces discontinuities. Returns 1 on success or 0 if an allocation fails.
    ...
    int16_t *values, int16_t min_value, int16_t last_value)
{
    ...
    if((low_freq == NULL) || (high_freq == NULL))
      {
        free(low_freq);
        free(high_freq);
        return 0;
    ...
    if((win_average_buffer(low_freq, filtered_count, WINDOW_LENGTH >> 1) == 0) ||
       (win_average_buffer(high_freq, filtered_count, WINDOW_LENGTH >> 1) == 0))
      {
        free(low_freq);
        free(high_freq);
        return 0;
...
// For the ends, we use only the visible samples. Returns 0 if allocation fails,
// leaving samples unchanged, or 1 on success.
