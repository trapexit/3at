...
struct adpcm_context {
  ...
  // Raw padding makes decoder and encoder state diverge, so encoding must stop.
  unsigned char _raw_encode_finalized;
  /* Lookahead search nodes left for the sample being encoded. The recursive
   * search is exponential on highly transient content (over 2e7 nodes for one
   * sample of an impulse train), so each sample gets a fixed budget. Content
   * that stays under the budget is unaffected. */
  uint32_t nodes_left;
};

#define ADPCM_NODE_BUDGET 1000000
...
int adpcm_sample_count_to_block_size (int sample_count, int num_chans, int bps)
{
    ...
    if (sample_count < 1 || num_chans < 1 || num_chans > 2 || bps < 2 || bps > 5)
        return 0;

    block_size = (((uint64_t) (sample_count - 1) * bps + 31) / 32) * num_chans * 4 +
        num_chans * 4;

    return block_size <= INT_MAX ? (int) block_size : 0;
}
...
int adpcm_block_size_to_sample_count (int block_size, int num_chans, int bps)
{
    ...
    if (num_chans < 1 || num_chans > 2 || bps < 2 || bps > 5)
        return 0;

    header_size = num_chans * 4;

    if (block_size < header_size)
        return 0;

    sample_count = (uint64_t) (block_size - header_size) / num_chans * 8 / bps + 1;
    return sample_count <= INT_MAX ? (int) sample_count : 0;
}
...
int adpcm_sample_count_to_block_size_no_header (int sample_count, int num_chans, int bps)
{
    ...
    if (sample_count < 0 || num_chans < 1 || num_chans > 2 || bps < 2 || bps > 5)
        return 0;

    block_size = (((uint64_t) sample_count * bps + 31) / 32) * num_chans * 4;
    return block_size <= INT_MAX ? (int) block_size : 0;
}
...
int adpcm_block_size_to_sample_count_no_header (int block_size, int num_chans, int bps)
{
    ...
    if (block_size < 0 || num_chans < 1 || num_chans > 2 || bps < 2 || bps > 5)
        return 0;

    sample_count = (uint64_t) block_size / num_chans * 8 / bps;
    return sample_count <= INT_MAX ? (int) sample_count : 0;
}
...
{
    uint64_t aligned_count;
    int sample_count = adpcm_block_size_to_sample_count (block_size, num_chans, bps);
    int sample_align;

    if (!sample_count || (round_up != 0 && round_up != 1))
        return 0;

    sample_align = (bps & 1) ? 32 : 32 / bps;
    aligned_count = ((uint64_t) sample_count - 1 +
        (sample_align - 1) * round_up) / sample_align * sample_align;

    if (aligned_count >= INT_MAX)
        return 0;

    return adpcm_sample_count_to_block_size ((int) aligned_count + 1, num_chans, bps);
}
...
int adpcm_align_block_size_no_header (int block_size, int num_chans, int bps, int round_up)
{
    ...
    if (block_size < 0 || num_chans < 1 || num_chans > 2 ||
        bps < 2 || bps > 5 || (round_up != 0 && round_up != 1))
        return 0;

    sample_count = adpcm_block_size_to_sample_count_no_header (block_size, num_chans, bps);
    sample_align = (bps & 1) ? 32 : 32 / bps;
    aligned_count = ((uint64_t) sample_count +
        (sample_align - 1) * round_up) / sample_align * sample_align;

    if (aligned_count > INT_MAX)
        return 0;

    return adpcm_sample_count_to_block_size_no_header (
        (int) aligned_count, num_chans, bps);
}
...
{
    struct adpcm_context *pcnxt;
    int ch;

    if (num_channels < 1 || num_channels > 2)
        return NULL;

    pcnxt = malloc (sizeof (struct adpcm_context));
...
void adpcm_set_shaping_weight (void *p, double shaping_weight)
{
    ...
    if (isnan (shaping_weight))
        pcnxt->static_shaping_weight = 0;
    else if (shaping_weight >= 1.0)
        pcnxt->static_shaping_weight = 1024;
    else if (shaping_weight <= -1.0)
 ...
 *  nodes           remaining search nodes for this sample; depth-first search
 *                  stops branching once the budget is exhausted so that highly
 *                  transient content cannot recurse without bound
 *
 * Returns RMS total error for the specified depth
 */

static rms_error_t min_error_4bit (const struct adpcm_channel *pchan, int nch, int32_t csample, const int16_t *psample, int flags, int *best_nibble, rms_error_t max_error, uint32_t *nodes)
{
    ...
    // if we're at a leaf, out of search budget, or we're not at a leaf but have
    // already exceeded the error limit, return
    if (!(flags & LOOKAHEAD_DEPTH) || (*nodes == 0) || min_error >= max_error)
        return min_error;
    ...
    min_error += min_error_4bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, max_error - min_error, nodes);
    ...
    if ((flags & LOOKAHEAD_NO_BRANCHING) || (*nodes == 0))
        return min_error;
    ...
    for (testnbl = 0; testnbl <= 0xF; ++testnbl) {
        ...
        if (flags & LOOKAHEAD_EXHAUSTIVE || !(~testnbl & 0x7) || abs (NIBBLE_TO_DELTA (4,nibble) - NIBBLE_TO_DELTA (4,testnbl)) <= 3) {
            ...
            if (error < threshold) {
                ...
                error += min_error_4bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, threshold - error, nodes);
...
static rms_error_t min_error_2bit (const struct adpcm_channel *pchan, int nch, int32_t csample, const int16_t *psample, int flags, int *best_nibble, rms_error_t max_error, uint32_t *nodes)
{
    int32_t delta = csample - pchan->pcmdata, csample2;
    struct adpcm_channel chan = *pchan;
    uint16_t step = step_table[chan.index];
    ...
    // if we're at a leaf, out of search budget, or already over the error limit, return
    if (!(flags & LOOKAHEAD_DEPTH) || (*nodes == 0) || min_error >= max_error)
        return min_error;
    ...
    min_error += min_error_2bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, max_error - min_error, nodes);
    ...
    for (testnbl = 0; testnbl <= 0x3; ++testnbl) {
        ...
        if (error < threshold) {
            ...
            error += min_error_2bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, threshold - error, nodes);
...
static rms_error_t min_error_3bit (const struct adpcm_channel *pchan, int nch, int32_t csample, const int16_t *psample, int flags, int *best_nibble, rms_error_t max_error, uint32_t *nodes)
{
    int32_t delta = csample - pchan->pcmdata, csample2;
    struct adpcm_channel chan = *pchan;
    uint16_t step = step_table[chan.index];
    ...
    min_error += min_error_3bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, max_error - min_error, nodes);
    ...
    for (testnbl = 0; testnbl <= 0x7; ++testnbl) {
        ...
        if (flags & LOOKAHEAD_EXHAUSTIVE || !(~testnbl & 0x3) || abs (NIBBLE_TO_DELTA (3,nibble) - NIBBLE_TO_DELTA (3,testnbl)) <= 2) {
            ...
            if (error < threshold) {
                ...
                error += min_error_3bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, threshold - error, nodes);
...
static rms_error_t min_error_5bit (const struct adpcm_channel *pchan, int nch, int32_t csample, const int16_t *psample, int flags, int *best_nibble, rms_error_t max_error, uint32_t *nodes)
{
    ...
    min_error += min_error_5bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, max_error - min_error, nodes);
    ...
    for (testnbl = 0; testnbl <= 0x1F; ++testnbl) {
        ...
        if (flags & LOOKAHEAD_EXHAUSTIVE || (testnbl | 3) == (nibble | 0xf) || abs (NIBBLE_TO_DELTA (5,nibble) - NIBBLE_TO_DELTA (5,testnbl)) <= 3) {
            ...
            if (error < threshold) {
                ...
                error += min_error_5bit (&chan, nch, csample2, psample + nch, flags - 1, NULL, threshold - error, nodes);
...
static uint8_t encode_sample (struct adpcm_context *pcnxt, int ch, int bps, const int16_t *psample, int num_samples)
{
    ...
    pcnxt->nodes_left = ADPCM_NODE_BUDGET;

    if (bps == 2) {
        min_error_2bit (pchan, pcnxt->num_channels, csample, psample, flags, &nibble, MAX_RMS_ERROR, &pcnxt->nodes_left);
        ...
        min_error_3bit (pchan, pcnxt->num_channels, csample, psample, flags, &nibble, MAX_RMS_ERROR, &pcnxt->nodes_left);
        trial_delta = (step >> 2);
        if (nibble & 1) trial_delta += (step >> 1);
        if (nibble & 2) trial_delta += step;
        ...
        min_error_4bit (pchan, pcnxt->num_channels, csample, psample, flags, &nibble, MAX_RMS_ERROR, &pcnxt->nodes_left);
        trial_delta = (step >> 3);
        if (nibble & 1) trial_delta += (step >> 2);
        if (nibble & 2) trial_delta += (step >> 1);
        if (nibble & 4) trial_delta += step;
        ...
        min_error_5bit (pchan, pcnxt->num_channels, csample, psample, flags, &nibble, MAX_RMS_ERROR, &pcnxt->nodes_left);
        trial_delta = (step >> 4);
        if (nibble & 1) trial_delta += (step >> 3);
        if (nibble & 2) trial_delta += (step >> 2);
        if (nibble & 4) trial_delta += (step >> 1);
...
static void encode_chunks (struct adpcm_context *pcnxt, uint8_t *outbuf, size_t *outbufsize, const int16_t *inbuf, int inbufcount, int bps)
{
    ...
    payload_chunks = (((uint64_t) inbufcount * bps + ADPCM_CHUNK_BITS - 1) /
                      ADPCM_CHUNK_BITS);
    payload_size = (size_t)(payload_chunks * pcnxt->num_channels * sizeof(uint32_t));
    memset (outbuf, 0, payload_size);
    ...
    *outbufsize += payload_size;
}

// Encode PCM samples into 2-5 bit ADPCM; inbufcount counts samples per channel.
// Return 1 on success, 0 for invalid arguments, allocation failure, or nonempty
// encoding after raw finalization. A non-NULL outbufsize receives the byte count.
// A successful raw call with a partial chunk preserves zero padding and finalizes
// encoding; only empty no-ops may follow. Headered blocks remain independent.
...
int adpcm_encode_block_ex (void *p, uint8_t *outbuf, size_t *outbufsize, const int16_t *inbuf, int inbufcount, int bps)
{
  ...
  *outbufsize = 0;
  if((pcnxt == NULL) || (inbufcount < 0) || (bps < 2) || (bps > 5) ||
     ((inbufcount != 0) && ((outbuf == NULL) || (inbuf == NULL))))
    return 0;

  if(inbufcount == 0)
    return 1;

  if(pcnxt->_raw_encode_finalized)
    {
      return 0;
    }
        ...
        if (inbufcount && (pcnxt->channels [0].index < 0 || (pcnxt->config_flags & LOOKAHEAD_DEPTH) >= 3)) {
            ...
            for (ch = 0; ch < pcnxt->num_channels; ch++) {
                ...
                for (tindex = 0; tindex <= 88; tindex++) {
                    ...
                    uint32_t nodes = ADPCM_NODE_BUDGET;

                    chan.index = tindex;
                    chan.shaping_weight = 0;

                    if (bps == 2)
                        error_per_index [tindex] = min_error_2bit (&chan, pcnxt->num_channels, inbuf [ch], inbuf + ch, flags, NULL, MAX_RMS_ERROR, &nodes);
                    else if (bps == 3)
                        error_per_index [tindex] = min_error_3bit (&chan, pcnxt->num_channels, inbuf [ch], inbuf + ch, flags, NULL, MAX_RMS_ERROR, &nodes);
                    else if (bps == 5)
                        error_per_index [tindex] = min_error_5bit (&chan, pcnxt->num_channels, inbuf [ch], inbuf + ch, flags, NULL, MAX_RMS_ERROR, &nodes);
                    else
                        error_per_index [tindex] = min_error_4bit (&chan, pcnxt->num_channels, inbuf [ch], inbuf + ch, flags, NULL, MAX_RMS_ERROR, &nodes);
                }
    ...
    if (inbufcount && (pcnxt->config_flags & NOISE_SHAPING_DYNAMIC)) {
        pcnxt->dynamic_shaping_array = malloc (
            (size_t) inbufcount * sizeof (int16_t));

        if (!pcnxt->dynamic_shaping_array ||
            !generate_dns_values (inbuf, inbufcount, pcnxt->num_channels,
                pcnxt->sample_rate, pcnxt->dynamic_shaping_array, -512,
                pcnxt->last_shaping_weight)) {
    ...
    if (inbufcount) {
        ...
        if ((pcnxt->config_flags & FORMAT_INTEL_DVI4) && bps == 4) {
            int bytes_per_channel = inbufcount / 2 + (inbufcount & 1);
            int i;

            for (ch = 0; ch < pcnxt->num_channels; ++ch)
                for (i = 0; i < bytes_per_channel; ++i) {
                    size_t offset = (size_t) (i & ~3) * pcnxt->num_channels +
                        ch * 4 + (i & 3);

                    outbuf [offset] =
                        (outbuf [offset] << 4) | ((outbuf [offset] >> 4) & 0xf);
                }
        }
    }
  ...
  if(pcnxt->config_flags & FORMAT_NO_HEADERS)
    {
      int sample_align = ((bps & 1) ? ADPCM_CHUNK_BITS : (ADPCM_CHUNK_BITS / bps));

      // A decoder consumes the padding codes, but encode_chunks() does not.
      // Continuing after padding would therefore use a different predictor state.
      pcnxt->_raw_encode_finalized = ((inbufcount % sample_align) != 0);
    }

    return 1;
}

// Four-bit wrapper; propagates failure and the raw finalization contract above.
...
int adpcm_decode_block (void *p, int16_t *outbuf, const uint8_t *inbuf, size_t inbufsize, int channels)
{
    ...
    if (channels < 1 || channels > 2 || !outbuf || !inbuf)
        return 0;

    no_headers = pcnxt && (pcnxt->config_flags & FORMAT_NO_HEADERS);

    if (no_headers && pcnxt->num_channels != channels)
        return 0;
    ...
    if (!no_headers) {
        ...
        for (ch = 0; ch < channels; ch++) {
            outbuf [ch] = pcmdata[ch] =
                (int16_t) (inbuf [0] | (inbuf [1] << 8));
            index[ch] = inbuf [2];
        ...
        for (ch = 0; ch < channels; ++ch) {
            pcmdata [ch] = pcnxt->channels [ch].pcmdata;
            index [ch] = pcnxt->channels [ch].index;
        }

    if (inbufsize % channels)
        return 0;

    if (inbufsize / channels > (size_t) (INT_MAX - samples) / 2)
        return 0;

    while (inbufsize) {
        size_t bytes_per_channel = inbufsize / channels;
        ...
        bytes_in_chunk = bytes_per_channel * channels;

        for (ch = 0; ch < channels; ++ch)
            for (i = 0; i < (int) bytes_per_channel; ++i) {
                uint16_t step = step_table [index [ch]], delta = step >> 3;
                uint8_t inbyte = inbuf [ch * bytes_per_channel + i];
                ...
                outbuf [(i * 2) * channels + ch] = pcmdata[ch];
                ...
                CLIP(index[ch], 0, 88);
                CLIP(pcmdata[ch], -32768, 32767);
                outbuf [(i * 2 + 1) * channels + ch] = pcmdata[ch];
            }

        inbuf += bytes_in_chunk;
        inbufsize -= bytes_in_chunk;
        outbuf += bytes_per_channel * 2 * channels;
        samples += bytes_per_channel * 2;
    }

    if (no_headers)
        for (ch = 0; ch < channels; ++ch) {
            pcnxt->channels [ch].pcmdata = pcmdata [ch];
            pcnxt->channels [ch].index = index [ch];
        }
...
int adpcm_decode_block_ex (void *p, int16_t *outbuf, const uint8_t *inbuf, size_t inbufsize, int channels, int bps)
{
    ...
    if (!inbufsize || (inbufsize % (channels * 4)))
        return samples;

    bytes_per_channel = (uint64_t)(inbufsize / channels);
    if (bytes_per_channel > (UINT64_MAX / 8))
        return 0;

    decoded_samples = (bytes_per_channel * 8 / bps);

    if (decoded_samples > (uint64_t) INT_MAX - (no_headers ? 0 : 1))
        return 0;

    samples = (int) decoded_samples;

    switch (bps) {
        case 2:
            for (ch = 0; ch < channels; ++ch) {
                int shiftbits = 0, numbits = 0;
                size_t i, j;

                for (j = i = 0; i < (size_t) samples; ++i) {
