    ...
    int16_t *values, int16_t min_value, int16_t last_value)
{
    ...
    if (sample_count < 0 || num_chans < 1 || num_chans > 2)
        return 0;
    ...
    filtered_count = sample_count - FILTER_LENGTH + 1;
    ...
    if (!win_average_buffer (low_freq, filtered_count, WINDOW_LENGTH >> 1) ||
...
static int win_average_buffer (float *samples, int sample_count, int half_width)
{
    ...
    for (i = 0; i < sample_count; ++i) {
        if ((sample_count - i) <= half_width + 1)
            k = sample_count;
        else
            k = i + half_width + 1;
